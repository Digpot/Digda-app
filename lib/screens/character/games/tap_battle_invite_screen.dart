...
/// 초대를 보내면 대결 화면(대기 상태)으로 전환되고, 상대에겐 알림이 간다.
class TapBattleInviteScreen extends StatefulWidget {
  const TapBattleInviteScreen({super.key});

  @override
  State<TapBattleInviteScreen> createState() => _TapBattleInviteScreenState();
}

class _TapBattleInviteScreenState extends State<TapBattleInviteScreen> {
  ...
  Future<void> _load() async {
    ...
    if (groupId == null) {
      setState(() {
        _loading = false;
        _errorMessage = '그룹에 들어간 뒤 탭배틀을 즐길 수 있어요.';
      });
      return;
    }
    setState(() {
  ...
  void _confirmInvite(Membership member) {
    ...
    showConfirmDialog(
      ...
      message: '탭배틀 대결을 신청할까요?\n15초 동안 더 많이 탭하는 사람이 승리해요!',
      confirmLabel: '초대',
  ...
  Future<void> _invite(Membership member) async {
    ...
    try {
      final game = await Di.minigameRepository.createTapBattle(
        groupRoomId: groupId,
      ...
      await Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) =>
              TapBattleScreen(gameId: game.gameId, initialGame: game),
        ),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            const CenterTitleHeader(title: '탭배틀 상대 고르기'),
